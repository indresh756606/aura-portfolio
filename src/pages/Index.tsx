...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("portfolios").select("*").eq("user_id", user.id).maybeSingle().then(async ({ data, error }) => {
      ...
      if (!data) {
        const created = await supabase.from("portfolios").insert({ user_id: user.id, personal: { email: user.email || "" }, slug: user.id.replaceAll("-", "").slice(0, 12) }).select("*").single();
        data = created.data;
      }
  ...
  const share = async () => { const url = `${window.location.origin}/p/${portfolio.slug || user.id.replaceAll("-", "").slice(0, 12)}`; await navigator.clipboard.writeText(url); toast.success("Share link copied."); };
